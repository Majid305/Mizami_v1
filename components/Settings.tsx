...
export const Settings: React.FC<SettingsProps> = ({ enableReminders, setEnableReminders, theme, setTheme }) => {
    ...
    const handleBackup = async () => {
        try {
            ...
            if (data.documents.length === 0 && data.checks.length === 0 && data.avis.length === 0) {
            ...
            a.href = url;
            a.download = `mizami_full_backup_${new Date().toISOString().split('T')[0]}.json`;
    ...
    const handleRestore = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                if (!raw) throw new Error("Fichier vide ou corrompu.");
                
                const data = JSON.parse(raw);
                const count = await restoreBackup(data);
                
                alert(`${count} éléments restaurés avec succès. L'application va redémarrer.`);
                window.location.reload();
            } catch (err: any) {
                console.error(err);
                alert("Échec de la restauration : " + (err.message || "format invalide."));
            } finally {
        ...
        reader.onerror = () => {
            alert("Erreur fatale de lecture du fichier.");
            setIsRestoring(false);
        };
        ...
        <div className="h-full bg-gray-50 dark:bg-slate-950 p-6 overflow-y-auto transition-colors duration-300">
            ...
            {isRestoring && (
                <div className="fixed inset-0 bg-black/60 z-[100] flex flex-col items-center justify-center text-white backdrop-blur-sm">
                    ...
                    <p className="font-bold text-xl uppercase tracking-widest">Restauration en cours...</p>
                </div>
            )}
            ...
            <div className="mt-8 text-center pb-6">
                <p className="text-[10px] text-gray-400 dark:text-gray-600 font-mono uppercase tracking-widest font-black">Mizami v1.5.0 • Expert Data Management</p>
            </div>
        </div>
    );
};
