...
export const CheckDashboard = ({ onEdit }: { onEdit: (check: RejectedCheck) => void }) => {
    ...
    const handleExportXLSX = () => {
        ...
        XLSX.writeFile(wb, "Mizami_Cheques_Export.xlsx");
    };
    ...
    const handleDelete = async (id: string, e: React.MouseEvent) => {
        ...
        if (!window.confirm("Supprimer définitivement ce chèque de la base ?")) {
            setMenuOpenId(null);
            return;
        }
        ...
        try {
            ...
            // Mise à jour immédiate et forcée du state local
    ...
    const handleDownloadPDF = async (check: RejectedCheck, e: React.MouseEvent) => {
        ...
        try {
            ...
            doc.save(`Mizami_Cheque_${check.numero_cheque}.pdf`);
        } catch (err) {
            console.error(err);
            alert("Erreur lors de la génération de l'PDF.");
        } finally {
    ...
    return (
        <div className="h-full bg-beurre dark:bg-lavande-deep flex flex-col transition-colors duration-500">
            {generatingPdf && (
                    ...
                    >
                        ...
                        {menuOpenId === check.id && (
                            <div className="absolute right-4 top-12 bg-white dark:bg-slate-900 shadow-2xl border border-gray-100 dark:border-slate-800 rounded-2xl z-30 flex flex-col w-48 py-2 animate-in fade-in zoom-in-95 duration-150">
                                <button onClick={(e) => { e.stopPropagation(); onEdit(check); setMenuOpenId(null); }} className="flex items-center px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-800 text-sm font-bold text-gray-700 dark:text-gray-100 transition-colors">
                                    <Edit size={16} className="mr-3 text-citron" /> Modifier
                                </button>
                                <button onClick={(e) => { e.stopPropagation(); handleDuplicate(check, e); }} className="flex items-center px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-800 text-sm font-bold text-gray-700 dark:text-gray-100 transition-colors">
                                    <Copy size={16} className="mr-3 text-blue-500" /> Dupliquer
                                </button>
                                <button onClick={(e) => { e.stopPropagation(); handleDownloadPDF(check, e); }} className="flex items-center px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-800 text-sm font-bold text-gray-700 dark:text-gray-100 transition-colors">
                                ...
                                <button onClick={(e) => handleDelete(check.id, e)} className="flex items-center px-4 py-2.5 hover:bg-red-50 dark:hover:bg-red-900/20 text-sm font-bold text-red-600 dark:text-red-400 transition-colors">
