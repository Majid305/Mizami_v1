...
import { Camera, Upload, Loader2, Save, RefreshCw, Wand2, FileText, ArrowLeft, Calendar, Clock, ChevronLeft, ChevronRight, X, Check, Edit3, Image as ImageIcon, Sparkles } from 'lucide-react';
import { DocumentData, DocLanguage, DocStatus, DocType } from '../types';
import { analyzeDocument, generateResponseDraft } from '../services/geminiService';
...
export const Scanner: React.FC<ScannerProps> = ({ onSave, onCancel, initialData }) => {
  ...
  const handleGenerateResponse = async () => {
    if (!formData || !aiInstructions) {
        alert("Veuillez saisir des instructions pour l'IA.");
        return;
    }
    setDrafting(true);
    try {
        const draft = await generateResponseDraft(formData, aiInstructions);
        setFormData(prev => prev ? ({ ...prev, reponse: draft }) : null);
    } catch (err) {
        console.error(err);
  ...
  if (analyzing) {
      return (
          <div className="flex flex-col items-center justify-center h-full space-y-4 bg-gray-50 dark:bg-slate-950 text-center">
              <Loader2 size={64} className="text-[#0ABAB5] animate-spin" />
              <div className="space-y-2">
                <p className="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tighter">Analyse IA en cours...</p>
                <p className="text-sm text-slate-500">Extraction des métadonnées administratives</p>
              </div>
          </div>
      )
  }
  ...
  return (
    <div className="h-full overflow-y-auto pb-32 bg-gray-50 dark:bg-slate-950">
            ...
            <div className="p-4 space-y-6">
                ...
                <div className={`space-y-4 ${formData.langue_document === DocLanguage.AR ? 'rtl' : 'ltr'}`} dir={formData.langue_document === DocLanguage.AR ? 'rtl' : 'ltr'}>
                    ...
                    <div className="bg-white dark:bg-slate-900 px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-700 shadow-sm focus-within:ring-2 ring-[#0ABAB5]">
                        ...
                        <select className="w-full text-gray-800 dark:text-gray-100 outline-none bg-transparent" value={formData.type_objet || DocType.AUTRE} onChange={(e: any) => setFormData({...formData, type_objet: e.target.value as any})}>
                    ...
                    <Input label="Autres Informations / Contact" value={formData.autres_infos} onChange={(v: string) => setFormData({...formData, autres_infos: v})} />
                    ...
                    <div className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm transition-all">
                        ...
                        >
                            ...
                            {formData.date_heure_rappel && (
                                <button onClick={(e: any) => { e.stopPropagation(); setFormData({...formData, date_heure_rappel: ""}); }} className="ml-auto text-red-500 p-1">
                                    <X size={14} />
                                </button>
                            )}
                        </div>
                    ...
                    {/* Nouvelle section Instructions IA pour la réponse */}
                    <div className="bg-teal-50 dark:bg-slate-900 p-4 rounded-xl border border-teal-200 dark:border-slate-700 space-y-3">
                        <label className="block text-xs font-bold text-[#0ABAB5] uppercase tracking-tight flex items-center gap-2">
                            <Sparkles size={14} /> Instructions pour l'IA (Réponse)
                        </label>
                        <textarea 
                            rows={2} 
                            className="w-full bg-white dark:bg-slate-800 p-3 rounded-lg text-sm outline-none border border-transparent focus:border-[#0ABAB5] dark:text-white resize-none"
                            placeholder="Ex: Rédige une lettre de refus polie..."
                            value={aiInstructions}
                            onChange={(e) => setAiInstructions(e.target.value)}
                        />
                        <button 
                            onClick={handleGenerateResponse}
                            disabled={drafting || !aiInstructions}
                            className="w-full bg-[#0ABAB5] text-white py-2 rounded-lg font-bold text-xs flex items-center justify-center gap-2 active:scale-95 transition-all disabled:opacity-30"
                        >
                    ...
                    <TextArea label="Réponse / Action réalisée" value={formData.reponse} onChange={(v: string) => setFormData({...formData, reponse: v})} rows={4} />
                    ...
                    <div className="fixed bottom-20 right-4 left-4 z-40">
                         <button onClick={handleSave} disabled={loading} className="w-full bg-slate-900 dark:bg-[#0ABAB5] text-white py-4 rounded-xl shadow-xl font-bold text-lg flex justify-center items-center space-x-2 active:scale-95 transition-transform disabled:opacity-50">
        ...
        {pickerConfig && (
            <CustomDateTimePicker label={pickerConfig.label} type={pickerConfig.type} initialValue={formData?.[pickerConfig.field] as string} onClose={() => setPickerConfig(null)} onSelect={(val: string) => { setFormData(prev => prev ? ({...prev, [pickerConfig.field]: val}) : null); setPickerConfig(null); }} />
        )}
    </div>
  );
};
    ...
    return (
        <div className="fixed inset-0 z-[70] flex flex-col justify-end bg-black/60 backdrop-blur-sm p-4">
             <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-sm mx-auto shadow-2xl border dark:border-slate-800 animate-in slide-in-from-bottom duration-300">
                ...
                <button onClick={handleConfirm} className="w-full bg-[#0ABAB5] text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center space-x-2 active:scale-95 transition-transform"><Check size={20} /><span>VALIDER</span></button>
             </div>
        </div>
    );
};
